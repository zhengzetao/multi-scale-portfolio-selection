...
class RolloutBuffer(BaseBuffer):
    ...
    def reset(self) -> None:
        ...
        self.actions = np.zeros((self.buffer_size, self.agent_num, self.action_dim), dtype=np.float32)
        self.last_actions = np.zeros((self.buffer_size, self.agent_num, self.action_dim), dtype=np.float32)
        self.rewards = np.zeros((self.buffer_size, 1), dtype=np.float32)
        self.returns = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.episode_starts = np.zeros((self.buffer_size, 1), dtype=np.float32)
        self.values = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.log_probs = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.mean_actions = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.advantages = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.co_adv = np.zeros((self.buffer_size, self.agent_num, 1), dtype=np.float32)
        self.generator_ready = False
